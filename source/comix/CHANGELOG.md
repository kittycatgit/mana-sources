...
- Initial implementation, reading comix.to. The site is a React app over a Laravel API
  whose every listing route is signed per request by an obfuscated module versioned with
  the bundle, so nothing on that half can be called directly.
- The home page and the title view need no browser at all: every document route inlines the
  app's dehydrated query cache, which a plain request already carries. Home is four rows —
  Trending Today, Latest Updates, Most Followed and Recently Added — out of one request.
- Browse, chapters and pages come out of the running app instead: the source loads the page
  in a WebView and reads the decrypted reply out of React's fiber tree. That is about two
  seconds and no image downloads for a chapter, against a minute for scrolling the reader.
- The chapter list walks its own pager by clicking it, one navigation instead of one per
  page, so a title with 707 uploads across 36 pages comes back complete in about twenty
  seconds. A chapter number appears once per scanlation group and every upload is kept,
  with the group carried as the chapter's provider.
- Chapter dates are reconstructed from the site's relative wording — "1mo ago", "2w ago" —
  which is the only date its chapter payload carries.
- Search covers the keyword query, all thirteen of the site's sorts, and its type, status,
  demographic, format, content rating, genre include/exclude, genre matching mode, year
  range and minimum chapter count filters.
- Page images need comix.to as their referer or the CDN answers 403 with an error page.
